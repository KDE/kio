...
public:
    ...
    {
        Q_ASSERT(udsField & KIO::UDSEntry::UDS_STRING);
        auto it = std::find_if(storage.begin(), storage.end(), [udsField](const Field &field) {
    ...
    {
        return QStringLiteral("size:%1 space used:%2")
            .arg(storage.size() * sizeof(Field) + sizeof(std::vector<Field>) + sizeof(std::vector<NumberField>) + numberStorage.size() * sizeof(NumberField))
            .arg(sizeof(std::vector<Field>) + sizeof(std::vector<NumberField>) + storage.capacity() * sizeof(Field)
