...
void JobTest::copyPercentExcludesTheFileInHand()
{
    ...
    // file it is on when it starts and then reports on its own gate.
...
// filesystem is not eligible), detected via the "batchCopy" metadata the batch command always
// sets.
void JobTest::copyManyFilesBatched()
{
    ...
    if (!job->metaData().contains(QLatin1String("batchCopy"))) {
