...
QString KIOPrivate::KFileItemIconCache::iconForStandardPath(const QString &localDirectory)
{
...
void KIOPrivate::KFileItemIconCache::initializeStandardLocationsMap()
{
    const struct {
    ...
    for (const auto &row : mapping) {
        ...
        for (const QString &location : locations) {
            m_standardLocationsMap.insert(location, row.name);
        }
        ...
        if (!xdgUserDirs.isEmpty() && xdgUserDirsFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            while (!in.atEnd()) {
                ...
                if (line.startsWith(templatesLine)) {
                    ...
                    m_standardLocationsMap.insert(xdgTemplates, QStringLiteral("folder-templates"));
                    ...
                    m_standardLocationsMap.insert(xdgPublicShare, QStringLiteral("folder-public"));
                }
            }
        }
#endif
