...
static void verifyRunningJobsCount(const std::unordered_map<QString, std::unique_ptr<HostQueue>> *const queues, int runningJobsCount)
{
    Q_UNUSED(queues);
...
void ProtoQueue::queueJob(SimpleJob *job)
{
    ...
    auto hostnameQueuesIt = m_queuesByHostname.find(hostname);
    if (hostnameQueuesIt == m_queuesByHostname.end()) {
        auto pair = m_queuesByHostname.emplace(std::move(hostname), std::make_unique<HostQueue>());
        Q_ASSERT(pair.second);
        hostnameQueuesIt = pair.first;
    }
    HostQueue &hq = *hostnameQueuesIt->second.get();
...
void ProtoQueue::removeJob(SimpleJob *job)
{
    ...
    auto hostnameQueuesIt = m_queuesByHostname.find(jobPriv->m_url.host());
    if (hostnameQueuesIt == m_queuesByHostname.end()) {
        Q_ASSERT(false);
        return;
    }
