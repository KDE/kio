...
QString KProtocolManagerPrivate::proxyFor(const QString &protocol)
{
    ...
    if (index > -1)  {
        bool ok = false;
        const QStringView proxyStrView{proxyStr};
        const auto portStr = proxyStrView.right(proxyStrView.size() - index - 1);
        (void)portStr.toInt(&ok);
        if (ok) {
            proxyStr = proxyStrView.left(index) + QL1C(':') + portStr;
        } else {
            proxyStr.clear();
        }
    }
...
QStringList KProtocolManagerPrivate::getSystemProxyFor(const QUrl &url)
{
    ...
    if (!proxyVar.isEmpty()) {
        ...
        proxy = QL1S("socks://") + QStringView{proxy}.mid(offset);
        if (!proxy.isEmpty()) {
            proxies << proxy;
        }
    }
