...
{
    static const QString exec = QStandardPaths::findExecutable(QStringLiteral("testparm"));
    if (exec.isEmpty()) {
        return QString();
    }
...
QByteArray KSambaSharePrivate::getNetUserShareInfo()
{
    ...
    static const QString exec = QStandardPaths::findExecutable(QStringLiteral("net"));
    if (exec.isEmpty()) {
        return QByteArray();
    }
