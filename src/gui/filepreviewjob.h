...
class FilePreviewJob : public KIO::Job
{
    Q_OBJECT
public:
    FilePreviewJob(const KFileItem &fileItem, int fileDeviceId, const PreviewSetupData &setupData, const PreviewOptions &options);
    ~FilePreviewJob();
...
private:
    ...
    // device ID for original url.
    const int m_fileDeviceId;
...
inline FilePreviewJob *filePreviewJob(const KFileItem &fileItem, int fileDeviceId, const PreviewSetupData &setupData, const PreviewOptions &options)
{
    auto job = new FilePreviewJob(fileItem, fileDeviceId, setupData, options);
    return job;
}
