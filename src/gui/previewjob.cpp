...
void PreviewJobPrivate::startPreview()
{
    ...
    QObject::connect(pathsFileDeviceIdsJob, &KIO::Job::result, q, [this](KJob *job) {
        ...
        thumbRootDeviceId = deviceIdMap.value(thumbRoot);
...
void PreviewJobPrivate::determineNextFile()
{
    ...
    for (int i = 0; i < jobsToRun; i++) {
        ...
        item.fileDeviceId = deviceIdMap.value(parentDir);

        FilePreviewJob *job = KIO::filePreviewJob(item, thumbRoot, thumbRootDeviceId, mimeMap);
        q->addSubjob(job);
        job->start();
    }
}
