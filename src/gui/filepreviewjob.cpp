...
FilePreviewJob::FilePreviewJob(const PreviewItem &item, const QString &thumbRoot, int thumbRootDeviceId, const QMap<QString, KPluginMetaData> &mimeMap)
    : m_item(item)
    , m_size(m_item.size)
    ...
    , m_thumbRootDeviceId(thumbRootDeviceId)
...
FilePreviewJob::CachePolicy FilePreviewJob::canBeCached(const QString &path)
{
    ...
    if (m_item.fileDeviceId == m_idUnknown) {
        return CachePolicy::Unknown;
    }

    bool isDifferentSystem = !m_item.fileDeviceId || m_item.fileDeviceId != m_currentDeviceId;
    ...
    if (m_thumbRootDeviceId == m_idUnknown) {
        return CachePolicy::Unknown;
    }
    bool shouldAllow = checkedId && checkedId == m_thumbRootDeviceId;
    if (!shouldAllow) {
