...
void KDynamicJobTracker::registerJob(KJob *job)
{
    ...
    if (!d->jobViewServerWatcher) {
        ...
        connect(d->jobViewServerWatcher, &QDBusServiceWatcher::serviceOwnerChanged, this, [this] {
            d->jobViewServerSupport = Private::NeedsChecking;
        });
    }

    if (d->jobViewServerSupport == Private::NeedsChecking) {
        ...
        if (reply.type() == QDBusMessage::ErrorMessage || reply.arguments().count() != 1)  {
            ...
            d->jobViewServerSupport = Private::Error;
        } else {
            const QString introspectionData = reply.arguments().first().toString();

            if (Private::hasDBusInterface(introspectionData, QStringLiteral("org.kde.JobViewServerV2"))) {
                d->jobViewServerSupport = Private::V2Supported;
            } else {
                d->jobViewServerSupport = Private::V2NotSupported;
            }
        }
    }

    if (d->jobViewServerSupport == Private::V2Supported) {
    ...
    if (d->jobViewServerSupport != Private::Error) {
    ...
    if (canHaveWidgets) {
        bool needsWidgetTracker = d->jobViewServerSupport == Private::Error;
