...
public:
    ...
    QWidget *init(const KFileItemList &items, QWidget *parent, std::function<void()> &updateCallback) override
    {
        ...
        beforeAfter = new QComboBox(widget);
        beforeAfter->addItems({i18nc("@label as in insert before", "Before filename"), i18nc("@lable as in insert after", "After filename")});

        auto textLabel = new QLabel(i18nc("@info add text to a filename", "Add Text:"), widget);
        textLineEdit = new QLineEdit(widget);
        textLineEdit->setPlaceholderText(i18nc("@info placeholder text", "Text to add"));
        ...
        QObject::connect(beforeAfter, &QComboBox::currentIndexChanged, updateCallback);
        ...
        addTextLayout->addWidget(beforeAfter);
    ...
    const std::function<QString(const QStringView fileName)> renameFunction() override
    {
        ...
        const auto append = beforeAfter->currentIndex() == 1;
