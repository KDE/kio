    ...
    : QDialog(parent)
{
    ...
    m_lineEdit = new QLineEdit(suggestedFileName, frame);
    ...
    if (defaultFilename.isEmpty()) {
        defaultFilename = i18nc("A default file name excluding extension", "pasted file");
    }
    ...
    if (mimetype.isValid()) {
        if (defaultFilename.endsWith(mimetype.preferredSuffix())) {
            m_lineEdit->setSelection(0, defaultFilename.length() - mimetype.preferredSuffix().length() - 1);
        } else {
            m_lineEdit->setText(defaultFilename + QLatin1String(".") + mimetype.preferredSuffix());
            m_lineEdit->setSelection(0, defaultFilename.length());
        }
    }
    m_lineEdit->setFocus();
