    ...
    , m_isReadOnly(false)
{
    ...
    if (isDevice() && !isHidden() && m_access && device().vendor() == QLatin1String("fuse.sshfs")) {
        const QString storageFilePath = m_access->filePath();
        // kdeconnect mounts into runtime dir or temp, anyone else cannot be kdeconnect.
        ...
        if (!runtimePath.isEmpty()) {
            runtimePath.append(QLatin1Char('/'));
        }

        if (runtimePath.isEmpty() || storageFilePath.startsWith(runtimePath)) {
